...
//	Edge<T>		*edge;
	...
	Node(void);
	Node(const double min, const double max);
	...
	virtual double distance(const Node<T> *p2);
	virtual double distance2(const Node<T> *p2);
...
template <int T>
VectorXd Node<T>::mins(T);
template <int T>
VectorXd Node<T>::maxs(T);
...
template <int T>
Node<T>::Node(const double min, const double max)
{
	int i;
...
template <int T>
double Node<T>::distance(const Node *p2)
{
	double ret;
	double ssum;
	double xx, yy;
...
template <int T>
double Node<T>::distance2(const Node *p2)
{
	double ret;
	double ssum;
	double xx, yy;
...
//	Edge<T> **edges;
	...
	RRT(VectorXd min_, VectorXd max_, double step_ = 0.);

	virtual Node<T> *getNewNode(void);

	~RRT(void);
//	void iterate( void (*projection)(  VectorXd &, VectorXd & ) );
	void iterate( void );
	double findPath( const Node<T> &node );
...
//	numEdges = 0;
	...
	Node<T>  *p_init = getNewNode();
	p_init->q = VectorXd::Zero(T);

	addNode(p_init);
}
template <int T>
Node<T> *RRT<T>::getNewNode(void)
{
	return new Node<T>;
}
